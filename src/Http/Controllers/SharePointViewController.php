...
{
    /**
     * View file from SharePoint using signed URL (inline in browser)
     *
     * URL format: /sharepoint/view/{itemId}?expires={timestamp}&signature={hash}
     */
    public function __invoke(Request $request, string $itemId): StreamedResponse
    {
        ...
        return response()->stream(function () use ($stream) {
            ...
            'Content-Disposition' => 'inline; filename="' . $file['name'] . '"', // ← inline instead of attachment
     ...
     */
    protected function validateSignature(Request $request, string $itemId): void
    {
     ...
     */
    protected function getFileByItemId(string $itemId, $disk): ?array
    {
        try {
