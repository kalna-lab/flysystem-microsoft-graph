...
class SharePointDownloadController
{
    ...
    private function getFileByItemId(string $itemId, $disk): ?array
    {
        try {
            ...
            $response = $graphAdapter->graph->createRequest('GET', "/drives/{$graphAdapter->driveId}/items/{$itemId}")
                ->execute();
            ...
            if (isset($graphAdapter->prefixer)) {
                $path = $graphAdapter->prefixer->stripPrefix($path);
            }
